...
class FileList  extends Sprite
{
	...
	{
		#if (air)
			var _locFile = File.documentsDirectory.resolvePath(path);
			...
			for (a in _locDir) {
				if (a.isDirectory) {
					continue;
					//_locIcon = new FileInfoIcon("[DIR] " + a.name, a);
				} else {
					var extension:String = a.name.substr(a.name.length - 4, a.name.length);
					if (extension == ".sol") {
						_locIcon = new FileInfoIcon("[FLASH] " + a.name, a.name);
					} else if (extension == ".trk") {
						_locIcon = new FileInfoIcon("[LRA] " + a.name, a.name);
					} else {
						extension = a.name.substr(a.name.length - 5, a.name.length);
						if (extension == ".json") {
							_locIcon = new FileInfoIcon("[JSON] " + a.name, a.name);
						}
					}
				}
		...
		#elseif sys
			var _locDir:Array<String> = FileSystem.readDirectory(path);
			var _locIcon:FileInfoIcon = new FileInfoIcon("you dun goofed!");
			var iteration:Int = 0;
			for (a in _locDir) {
				if (FileSystem.isDirectory(path + "/" + a)) {
	...
	{
		FileMenu.current.update(e.currentTarget);
	}
	
	public function update_list() {
		for (a in iconList) {
			if (listcontainer.contains(a)) this.listcontainer.removeChild(a);
		}
		iconList = new Array();
		this.get_directory_list();
	}
