...
class WindowBox extends Sprite
{
	...
	{
		this.removeChild(windowBar);
		this.removeChild(barLabel);
	}
	function set_error() 
	{
		this.fill = 0xFFAAAA;
	...
	public function displayItems() {
		...
		widthTracker = 10;
		if (this.message != null) heightTracker = this.contains(this.message) ? this.message.height + 5 : 40;
		else heightTracker = 40;
		for (a in itemList) {
		...
		for (index in itemList) {
			...
			if (lastItem == null) {
				index.item.x = 10;
				if (this.message != null) index.item.y = this.contains(this.message) ? this.message.height + 40 : 40;
				else index.item.y = 40;
